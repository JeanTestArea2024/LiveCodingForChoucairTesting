...
public class PersonDetailTo implements Task {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        for (Map<String, String> row : data) {
            actor.attemptsTo(
                    SendKeys.of(row.get("firstName")).into(PersonDetailView.firstName),
                    SendKeys.of(row.get("middleName")).into(PersonDetailView.middleName),
                    SendKeys.of(row.get("lastName")).into(PersonDetailView.lastName)
            );
        }
